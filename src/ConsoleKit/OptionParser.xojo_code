...
Protected Class OptionParser
	#tag Method, Flags = &h0, Description = 41646420616E206F7074696F6E20746F20746865207061727365722E
		Sub AddOption(o As ConsoleKit.Option)
		  ...
		  If o.ShortKey <> "" And Dict.HasKey(o.ShortKey.Asc) Then
		    Raise New OptionException("You can't add the same short key more than once: " + o.ShortKey)
		  End If
		  If o.LongKey <> "" And Dict.HasKey(o.LongKey) Then
		    Raise New OptionException("You can't add the same long key more than once: " + o.LongKey)
		  End If
		  If o.ShortKey = "?" Then
		    Raise New OptionException("You can't add the key ""?"" This means ""help"" and has already been added for you")
		  End If
	...
	#tag Method, Flags = &h0
		Function ArrayValue(key As Variant) As Variant()
		  ...
		  Var v() As Variant
		  Var o As ConsoleKit.Option = OptionValue(key)
	...
	#tag Method, Flags = &h0
		Sub Constructor(appName As String = "", appDescription As String = "")
		  ...
		  Var helpOption As New ConsoleKit.Option("h", "help", "Show help", ConsoleKit.OptionTypes.Boolean)
	...
	#tag Method, Flags = &h0
		Function OptionValue(key As String) As ConsoleKit.Option
		  //
	...
	#tag Method, Flags = &h0
		Sub Parse(args() As String, firstArgIsExecutable As Boolean = True)
		  ...
		  While optIdx < args.LastIndex // args can be rewritten in the loop
		    ...
		    Var opt As ConsoleKit.Option = OptionValue(key)
		    If opt = Nil Then
		      //
		      ...
		      If opt = Nil Or opt.Type <> ConsoleKit.OptionTypes.Boolean Then
		        RaiseUnrecognizedKeyException(key)
		      Else
		        value = "No"
		      End If
		    ...
		    ElseIf opt.Type = ConsoleKit.OptionTypes.Boolean Then
		      value = "Yes"
		  ...
		  If Not Self.HelpRequested Then
		    ...
		    For Each o As ConsoleKit.Option In Options
	...
	#tag Method, Flags = &h21
		Private Sub RaiseInvalidKeyValueException(key As String, type As String)
		  Raise New OptionException("Invalid key value: " + KeyWithDashes(key) + " (" + type + ")")
		End Sub
	#tag EndMethod

	#tag Method, Flags = &h21
		Private Sub RaiseMissingKeyException(key As String)
		  Raise New OptionException("Missing option: " + KeyWithDashes(key))
		End Sub
	#tag EndMethod

	#tag Method, Flags = &h21
		Private Sub RaiseUnrecognizedKeyException(key As String)
		  Raise New OptionException("Unrecognized key: " + KeyWithDashes(key))
		End Sub
	#tag EndMethod
		...
		Sub ShowHelp(sectionTitle As String = "Help")
		  ...
		  For i As Integer = 0 To Options.LastIndex
		    ...
		    If opt.ShortKey <> "" Then
		      ...
		      If opt.Type <> ConsoleKit.OptionTypes.Boolean Then
	...
	#tag Method, Flags = &h21
		Private Function Value(key As Variant) As Variant
		  ...
		  If v <> Nil Then
		    Return ConsoleKit.Option(v).Value
		  End If
	...
	#tag ComputedProperty, Flags = &h0
		...
		#tag Getter
			Get
			  Var o As ConsoleKit.Option = OptionValue("help")
			  If o Is Nil Then
	...
	#tag Property, Flags = &h21
		Private Options() As ConsoleKit.Option
	#tag EndProperty
